package com.aylax.applock.widget.patternview;

import static com.aylax.applock.widget.patternview.PatternView.AspectRatio.ASPECT_RATIO_HEIGHT_BIAS;
import static com.aylax.applock.widget.patternview.PatternView.AspectRatio.ASPECT_RATIO_SQUARE;
import static com.aylax.applock.widget.patternview.PatternView.AspectRatio.ASPECT_RATIO_WIDTH_BIAS;
import static com.aylax.applock.widget.patternview.PatternView.PatternViewMode.AUTO_DRAW;
import static com.aylax.applock.widget.patternview.PatternView.PatternViewMode.CORRECT;
import static com.aylax.applock.widget.patternview.PatternView.PatternViewMode.WRONG;
...
import com.aylax.applock.widget.patternview.util.PatternUtil;
import com.aylax.applock.widget.patternview.util.ResourceUtil;
...
public class PatternView extends View {
  ...
  /**
   * The time (in millis) spend in animating each circle of a lock pattern if the animating mode is
   * set. The entire animation should take this constant the length of the pattern to complete.
   */
  ...
  private final List<PatternListener> mPatternListeners;
  ...
  /**
   * Lookup table for the dots of the pattern we are currently drawing. This will be the dots of the
   * complete pattern unless we are animating, in which case we use this to hold the dots we are
  ...
  public PatternView(Context context, AttributeSet attrs) {
    ...
    try {
      sDotCount = typedArray.getInt(R.styleable.PatternView_dotCount, DEFAULT_PATTERN_DOT_COUNT);
      mAspectRatioEnabled =
          typedArray.getBoolean(R.styleable.PatternView_aspectRatioEnabled, false);
      mAspectRatio = typedArray.getInt(R.styleable.PatternView_aspectRatio, ASPECT_RATIO_SQUARE);
      mPathWidth =
          (int)
              typedArray.getDimension(
                  R.styleable.PatternView_pathWidth,
                  ResourceUtil.getDimensionInPx(getContext(), R.dimen.pattern_lock_path_width));
      mNormalStateColor =
          typedArray.getColor(
              R.styleable.PatternView_normalStateColor,
              ResourceUtil.getColor(getContext(), R.color.white));
      mCorrectStateColor =
          typedArray.getColor(
              R.styleable.PatternView_correctStateColor,
              ResourceUtil.getColor(getContext(), R.color.white));
      mWrongStateColor =
          typedArray.getColor(
              R.styleable.PatternView_wrongStateColor,
              ResourceUtil.getColor(getContext(), R.color.pomegranate));
      mDotNormalSize =
          (int)
              typedArray.getDimension(
                  R.styleable.PatternView_dotNormalSize,
                  ResourceUtil.getDimensionInPx(getContext(), R.dimen.pattern_lock_dot_size));
      mDotSelectedSize =
          (int)
              typedArray.getDimension(
                  R.styleable.PatternView_dotSelectedSize,
                  ResourceUtil.getDimensionInPx(
                      getContext(), R.dimen.pattern_lock_dot_selected_size));
      mDotAnimationDuration =
          typedArray.getInt(
              R.styleable.PatternView_dotAnimationDuration, DEFAULT_DOT_ANIMATION_DURATION);
      mPathEndAnimationDuration =
          typedArray.getInt(
              R.styleable.PatternView_pathEndAnimationDuration,
  ...
  protected Parcelable onSaveInstanceState() {
    ...
    return new SavedState(
        superState,
        PatternUtil.patternToString(this, mPattern),
        mPatternViewMode,
        mInputEnabled,
        mInStealthMode,
  ...
  protected void onRestoreInstanceState(Parcelable state) {
    ...
    setPattern(CORRECT, PatternUtil.stringToPattern(this, savedState.getSerializedPattern()));
   ...
   * (path drawing, dot animating, etc) as the user enters the pattern
   */
   ...
   * while showing any message to the user so that the user cannot get the view in an unwanted state
   */
  ...
  /**
   * Set the pattern explicitly rather than waiting for the user to input a pattern. You can use
   * this for help or demo purposes
   *
  ...
  public void addPatternLockListener(PatternListener patternListener) {
    mPatternListeners.add(patternListener);
  }

  public void removePatternLockListener(PatternListener patternListener) {
  ...
  private void notifyListenersStarted() {
    for (PatternListener patternListener : mPatternListeners) {
   ...
   * finding the dot, also makes heuristic choices such as filling in gaps based on current
   * pattern).
   *
  ...
  private void startLineEndAnimation(
  ...
  private void drawCircle(
      Canvas canvas, float centerX, float centerY, float size, boolean partOfPattern, float alpha) {
  ...
  public @interface PatternViewMode {
    /**
     * This state represents a correctly drawn pattern by the user. The color of the path and the
     * dots both would be changed to this color.
     *
     * <p>(NOTE - Consider showing this state in a friendly color)
     */
    int CORRECT = 0;
    /** Automatically draw the pattern for demo or tutorial purposes. */
    int AUTO_DRAW = 1;
    /**
     * This state represents a wrongly drawn pattern by the user. The color of the path and the dots
     * both would be changed to this color.
     *
     * <p>(NOTE - Consider showing this state in an attention-seeking color)
     */
    int WRONG = 2;
  }
  ...
  interface PatternListener {
    ...
    void onProgress(List<Dot> pattern);
  ...
  public static class Dot implements Parcelable {
    ...
    public static final Creator<Dot> CREATOR =
        new Creator<Dot>() {
