package com.aylax.applock.widget.patternview.util;

import com.aylax.applock.widget.patternview.PatternView;
...
public class PatternUtil {
     ...
     */
    public static String patternToString(PatternView patternLockView,
                                         List<PatternView.Dot> pattern) {
        if (pattern == null) {
            return "";
        }
        ...
        for (int i = 0; i < patternSize; i++) {
            PatternView.Dot dot = pattern.get(i);
     ...
     */
    public static List<PatternView.Dot> stringToPattern(PatternView patternLockView,
                                                            String string) {
        List<PatternView.Dot> result = new ArrayList<>();

        for (int i = 0; i < string.length(); i++) {
            int number = Character.getNumericValue(string.charAt(i));
            result.add(PatternView.Dot.of(number / patternLockView.getDotCount(),
     ...
     */
    public static String patternToSha1(PatternView patternLockView,
                                       List<PatternView.Dot> pattern) {
        try {
     ...
     */
    public static String patternToMD5(PatternView patternLockView,
                                      List<PatternView.Dot> pattern) {
        try {
     ...
     */
    public static ArrayList<PatternView.Dot> generateRandomPattern(PatternView patternLockView,
                                                                       int size)
            throws IndexOutOfBoundsException {
        if (patternLockView == null) {
            throw new IllegalArgumentException("PatternView can not be null.");
        }
        ...
        ArrayList<PatternView.Dot> result = new ArrayList<>();
        for (int id : usedIds) {
            result.add(PatternView.Dot.of(id));
        }
